...
export default function DeleteScheduleModal({ schedule, onClose, onDeleted }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl border overflow-hidden" style={{ borderColor: 'hsl(213,30%,91%)' }}>
        <div className="px-6 py-6 text-center">
          ...
          <h2 className="text-sm font-bold mb-1" style={{ color: 'hsl(220,60%,15%)' }}>Delete Schedule</h2>
          <p className="text-xs" style={{ color: 'hsl(220,15%,55%)' }}>
            Remove <span className="font-semibold" style={{ color: 'hsl(220,60%,15%)' }}>{DAYS[schedule.day_of_week]}</span> schedule for{' '}
            <span className="font-semibold" style={{ color: 'hsl(220,60%,15%)' }}>
              Dr. {schedule.dentist?.first_name} {schedule.dentist?.last_name}
