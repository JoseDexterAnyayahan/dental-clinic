...
import { Dentist, Paginated } from '@/types/admin'
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayed.map((d) => (
            ...
            >
              ...
              {d.email && (
                <p className="text-xs truncate" style={{ color: 'hsl(220,15%,55%)' }}>{d.email}</p>
              )}
              {d.bio && (
                <p className="text-xs mt-2 line-clamp-2" style={{ color: 'hsl(220,15%,60%)' }}>{d.bio}</p>
              )}
            </div>
          ))}
        </div>
